...
// @ref(zh): https://github.com/programmer10110/gostreebog
...
25, 61, 255, 53, 138, 126, 109, 84, 198, 128, 195, 189, 13, 87, 223, 245, 36, 
169, 62, 168, 67, 201, 215, 121, 214, 246, 124, 34, 185, 3, 224, 15, 236, 222, 
122, 148, 176, 188, 220, 232, 40, 80, 78, 51, 10, 74, 167, 151, 96, 115, 30, 0, 
98, 68, 26, 184, 56, 130, 100, 159, 38, 65, 173, 69, 70, 146, 39, 94, 85, 47, 
140, 163, 165, 125, 105, 213, 149, 59, 7, 88, 179, 64, 134, 172, 29, 247, 48, 
55, 107, 228, 136, 217, 231, 137, 225, 27, 131, 73, 76, 63, 248, 254, 141, 83, 
170, 144, 202, 216, 133, 97, 32, 113, 103, 164, 45, 43, 9, 91, 203, 155, 37, 
208, 190, 229, 108, 82, 89, 166, 116, 210, 230, 244, 180, 192, 209, 102, 175, 
194, 57, 75, 99, 182};

STREEBOG_TAU := []byte {
0, 8, 16, 24, 32, 40, 48, 56, 1, 9, 17, 25, 33, 41, 49, 57, 2, 10, 18, 26, 34, 
...
streebog_s :: inline proc "contextless"(x: []byte) -> []byte {
    for i := 0; i < 64; i += 1 do x[i] = STREEBOG_SBOX[x[i]];
    return x;
}

streebog_p :: inline proc "contextless"(x: []byte) -> []byte {
    b := make([]byte, 64);
    for i := 0; i < 64; i += 1 do b[i] = x[STREEBOG_TAU[i]];
    return b;
}

streebog_l :: inline proc "contextless"(x: []byte) -> []byte {
    v: u64;
    for i := 0; i < 8; i += 1 {
        v = 0;
        for k := 0; k < 8; k += 1 {
            for j := 0; j < 8; j += 1 {
                if (x[i * 8 + k] & (1 << uint(7 - j)) != 0) do v ~= STREEBOG_A[k * 8 + j];
            }
        }
        for k := 0; k < 8; k += 1 {
            x[i * 8 + k] = byte((v & u64(0xff << uint((7 - k) * 8))) >> uint((7 - k) * 8));
        }
    }
    return x;
}

streebog_xor512 :: inline proc "contextless"(a, b: []byte) -> []byte {
    c := make([]byte, 64);
    for i := 0; i < 64; i += 1 do c[i] = a[i] ~ b[i];
    return c;
}

streebog_k :: inline proc "contextless"(K: []byte, i: int) -> []byte {
	K = streebog_xor512(K, STREEBOG_C[i]);
	K = streebog_s(K);
	K = streebog_p(K);
	K = streebog_l(K);
	return K;
...
streebog_e :: inline proc "contextless"(K, m: []byte) -> []byte {
    b := streebog_xor512(K, m);
    for i := 0; i < 12; i += 1 {
        b = streebog_s(b);
		b = streebog_p(b);
		b = streebog_l(b);
		K = streebog_k(K, i);
		b = streebog_xor512(b, K);
    }
    return b;
}

streebog_gN :: inline proc "contextless"(N, h, m: []byte) -> []byte {
    K := streebog_xor512(h, N);
	K = streebog_s(K);
	K = streebog_p(K);
	K = streebog_l(K);
	t := streebog_e(K, m);
	t = streebog_xor512(h, t);
	G := streebog_xor512(t, m);
	return G;
}

streebog_addModulo512 :: inline proc "contextless"(a, b: []byte) -> []byte {
    t: uint;
	t = 0;
	c := make([]byte, 64);
	for i := 63; i >= 0; i -= 1 {
...
streebog_hash :: proc(data, IV: []byte) -> []byte {
    h := IV;
    N := make([]byte, 64);
    Sigma := make([]byte, 64);
    v512 := make([]byte, 64);
    ...
    for length >= 512 {
		m := data[length / 8 - 64:];
		h = streebog_gN(N, h, m);
		N = streebog_addModulo512(N, v512);
		Sigma = streebog_addModulo512(Sigma, m);
	...
	N = streebog_addModulo512(N, v512);
	Sigma = streebog_addModulo512(Sigma, m);
	h = streebog_gN(v0, h, N);
	h = streebog_gN(v0, h, Sigma);
...
streebog_256 :: proc "contextless" (data: []byte) -> [32]byte #no_bounds_check {
    hash : [32]byte;
    IV := make([]byte, 64);
    for i := 0; i < 64; i += 1 do IV[i] = 0x1;
    tmp := streebog_hash(data, IV); 
    copy(hash[:], tmp[:32]);
    return hash;
}

...
ripemd_block :: inline proc "contextless"(ctx: ^$T, p: []byte) -> int {
    ...
    when T == RIPEMD_160 do return ripemd_160_block(ctx, p);
    when T == RIPEMD_256 do return ripemd_256_block(ctx, p);
    when T == RIPEMD_320 do return ripemd_320_block(ctx, p);
    return 0;
}
...
ripemd_128_block :: proc(ctx: ^$T, p: []byte) -> int {
	...
	for len(p) >= RIPEMD_128_BLOCK_SIZE {
		...
		for i < 16 {
			...
			a, b, c, d = d, alpha, b, c;
			alpha = aa + (cc ~ (dd & (bb~cc))) + x[RIPEMD_128_N1[i]] + 0x50a28be6;
...
ripemd_checksum :: proc(ctx: ^$T, p: []byte) -> []byte {
    ...
    if tc % 64 < 56 {
        ripemd_write(ctx, tmp[0:56 - tc % 64]);
    } else {
        ripemd_write(ctx, tmp[0:64 + 56 - tc % 64]);
    }
    ...
    ripemd_write(ctx, tmp[0:8]);
    assert(ctx.nx == 0); // @note(zh): remove after thorough testing?
    size : int;
    when T == RIPEMD_128 {

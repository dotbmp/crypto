package rc2

import "../util"

// @ref(zh): https://github.com/dgryski/go-rc2

PI_TABLE := [256]byte {
	,
};

expand_key :: proc(key: []byte) -> [64]u16 {
    l := make([]byte, 128);
    defer delete(l);
    copy(l[:], key[:]);

    t := len(key);
	t1 := t * 8;
    t8 := (t1 + 7) / 8;
    tm := byte(255 % uint(1 << (8 + uint(t1) - 8 * uint(t8))));

    for i := t; i < 128; i += 1 do l[i] = PI_TABLE[l[i-1] + l[u8(i - t)]];

    l[128 - t8] = PI_TABLE[l[128 - t8] & tm];

    for i := 127 - t8; i >= 0; i -= 1 do l[i] = PI_TABLE[l[i + 1] ~ l[i + t8]];

    k: [64]u16;

    for _, i in k do k[i] = u16(l[2 * i]) + u16(l[2 * i + 1]) * 256;

	return k;
}

uint16 :: #force_inline proc "contextless"(b: []byte) -> u16 {
	return u16(b[0]) | u16(b[1]) << 8;
}

putuint16 :: #force_inline proc "contextless"(b: []byte, v: u16) {
	b[0] = byte(v);
	b[1] = byte(v >> 8);
}

encrypt :: proc(key, plaintext: []byte) -> [8]byte {
	expanded_key := expand_key(key);

	r0 := uint16(plaintext[0:]);
	r1 := uint16(plaintext[2:]);
	r2 := uint16(plaintext[4:]);
	r3 := uint16(plaintext[6:]);

	j: int;
	for j <= 16 {
		r0 = r0 + expanded_key[j] + (r3 & r2) + ((~r3) & r1);
		r0 = util.ROTL16(r0, 1);
		j += 1;

		r1 = r1 + expanded_key[j] + (r0 & r3) + ((~r0) & r2);
		r1 = util.ROTL16(r1, 2);
		j += 1;

		r2 = r2 + expanded_key[j] + (r1 & r0) + ((~r1) & r3);
		r2 = util.ROTL16(r2, 3);
		j += 1;

		r3 = r3 + expanded_key[j] + (r2 & r1) + ((~r2) & r0);
		r3 = util.ROTL16(r3, 5);
		j += 1;
	}

	r0 = r0 + expanded_key[r3 & 63];
	r1 = r1 + expanded_key[r0 & 63];
	r2 = r2 + expanded_key[r1 & 63];
	r3 = r3 + expanded_key[r2 & 63];

	for j <= 40 {
		r0 = r0 + expanded_key[j] + (r3 & r2) + ((~r3) & r1);
		r0 = util.ROTL16(r0, 1);
		j += 1;

		r1 = r1 + expanded_key[j] + (r0 & r3) + ((~r0) & r2);
		r1 = util.ROTL16(r1, 2);
		j += 1;

		r2 = r2 + expanded_key[j] + (r1 & r0) + ((~r1) & r3);
		r2 = util.ROTL16(r2, 3);
		j += 1;

		r3 = r3 + expanded_key[j] + (r2 & r1) + ((~r2) & r0);
		r3 = util.ROTL16(r3, 5);
		j += 1;
	}

	r0 = r0 + expanded_key[r3 & 63];
	r1 = r1 + expanded_key[r0 & 63];
	r2 = r2 + expanded_key[r1 & 63];
	r3 = r3 + expanded_key[r2 & 63];

	for j <= 60 {
		r0 = r0 + expanded_key[j] + (r3 & r2) + ((~r3) & r1);
		r0 = util.ROTL16(r0, 1);
		j += 1;

		r1 = r1 + expanded_key[j] + (r0 & r3) + ((~r0) & r2);
		r1 = util.ROTL16(r1, 2);
		j += 1;

		r2 = r2 + expanded_key[j] + (r1 & r0) + ((~r1) & r3);
		r2 = util.ROTL16(r2, 3);
		j += 1;

		r3 = r3 + expanded_key[j] + (r2 & r1) + ((~r2) & r0);
		r3 = util.ROTL16(r3, 5);
		j += 1;
	}

	cipher: [8]byte;

	putuint16(cipher[0:], r0);
	putuint16(cipher[2:], r1);
	putuint16(cipher[4:], r2);
	putuint16(cipher[6:], r3);

	return cipher;
}

decrypt :: proc(key, ciphertext: []byte) -> [8]byte {
	expanded_key := expand_key(key);

	r0 := uint16(ciphertext[0:]);
	r1 := uint16(ciphertext[2:]);
	r2 := uint16(ciphertext[4:]);
	r3 := uint16(ciphertext[6:]);

	j := 63;

	for j >= 44 {
		r3 = util.ROTL16(r3, 16 - 5);
		r3 = r3 - expanded_key[j] - (r2 & r1) - ((~r2) & r0);
		j -= 1;

		r2 = util.ROTL16(r2, 16 - 3);
		r2 = r2 - expanded_key[j] - (r1 & r0) - ((~r1) & r3);
		j -= 1;

		r1 = util.ROTL16(r1, 16 - 2);
		r1 = r1 - expanded_key[j] - (r0 & r3) - ((~r0) & r2);
		j -= 1;

		r0 = util.ROTL16(r0, 16 - 1);
		r0 = r0 - expanded_key[j] - (r3 & r2) - ((~r3) & r1);
		j -= 1;
	}

	r3 = r3 - expanded_key[r2 & 63];
	r2 = r2 - expanded_key[r1 & 63];
	r1 = r1 - expanded_key[r0 & 63];
	r0 = r0 - expanded_key[r3 & 63];

	for j >= 20 {
		r3 = util.ROTL16(r3, 16 - 5);
		r3 = r3 - expanded_key[j] - (r2 & r1) - ((~r2) & r0);
		j -= 1;

		r2 = util.ROTL16(r2, 16 - 3);
		r2 = r2 - expanded_key[j] - (r1 & r0) - ((~r1) & r3);
		j -= 1;

		r1 = util.ROTL16(r1, 16 - 2);
		r1 = r1 - expanded_key[j] - (r0 & r3) - ((~r0) & r2);
		j -= 1;

		r0 = util.ROTL16(r0, 16 - 1);
		r0 = r0 - expanded_key[j] - (r3 & r2) - ((~r3) & r1);
		j -= 1;
	}

	r3 = r3 - expanded_key[r2 & 63];
	r2 = r2 - expanded_key[r1 & 63];
	r1 = r1 - expanded_key[r0 & 63];
	r0 = r0 - expanded_key[r3 & 63];
	
	for j >= 0 {
		r3 = util.ROTL16(r3, 16 - 5);
		r3 = r3 - expanded_key[j] - (r2 & r1) - ((~r2) & r0);
		j -= 1;

		r2 = util.ROTL16(r2, 16 - 3);
		r2 = r2 - expanded_key[j] - (r1 & r0) - ((~r1) & r3);
		j -= 1;

		r1 = util.ROTL16(r1, 16 - 2);
		r1 = r1 - expanded_key[j] - (r0 & r3) - ((~r0) & r2);
		j -= 1;

		r0 = util.ROTL16(r0, 16 - 1);
		r0 = r0 - expanded_key[j] - (r3 & r2) - ((~r3) & r1);
		j -= 1;
	}

	plaintext: [8]byte;
	
	putuint16(plaintext[0:], r0);
	putuint16(plaintext[2:], r1);
	putuint16(plaintext[4:], r2);
	putuint16(plaintext[6:], r3);

    return plaintext;
}